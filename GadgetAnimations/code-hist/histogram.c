...
/*
    for (x = 0; x < Nbins; x++)
    {
        for (y = 0; y < Nbins; y++)
        {
            for (z = 0; z < Nbins; z++)
            {
                if (histogram1.grid[x][y][z] > 0.9)
                    histogram1.grid[x][y][z] = log(histogram1.grid[x][y][z]);
            }
        }
    }
*/
    ...
    {
        printf("Maximum = %f \t Minimum = %f \n", get_histogram_max(histogram1), get_histogram_min(histogram1));
    }
